...
duration_B_T = list(db['PermanentBookings'].aggregate(pip_d(), allowDiskUse=True))
duration_B_P = list(db['PermanentBookings'].aggregate(pip_d(city='Portland'), allowDiskUse=True))
duration_B_Te = list(db['enjoy_PermanentBookings'].aggregate(pip_d(city='Torino'), allowDiskUse=True))

duration_P_T = list(db['PermanentParkings'].aggregate(pip_d(city='Torino'), allowDiskUse=True))
duration_P_P = list(db['PermanentParkings'].aggregate(pip_d(city='Portland'), allowDiskUse=True))
duration_P_Te = list(db['enjoy_PermanentParkings'].aggregate(pip_d(city='Torino'), allowDiskUse=True))
...
plt.xscale("log")
plt.grid(linestyle='--', linewidth=.4, which="both")
plt.savefig(fname='plots/ECDF_P_Bookings_Parkings.png')
